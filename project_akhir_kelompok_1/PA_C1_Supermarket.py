...
def beli_barang(username):
    ...
    data_produk[produk_index][3] = str(stok_produk)
    ...
    tabel_invoice.field_names = ["Keterangan", "Detail"]
    tabel_invoice.align["Keterangan"] = "l"
    tabel_invoice.align["Detail"] = "l"

    tabel_invoice.add_row(["Nama Pembeli", username])
    tabel_invoice.add_row(["Nama Produk", nama_produk])
    tabel_invoice.add_row(["Jumlah Pembelian", f"{jumlah} pcs"])
    tabel_invoice.add_row(["Total Harga", f"Rp {total_harga:,}"])
    tabel_invoice.add_row(["Sisa Saldo", f"Rp {saldo_user:,}"])

    print(tabel_invoice)
    print("✅ Transaksi Berhasil!")
    print("Terima kasih telah berbelanja di K3A Mart ❤️\n")
